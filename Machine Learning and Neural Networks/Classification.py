...
class Classifier_model:
    def __init__(self, observer, predictor, k=3): #test and train consist of classifier and observation variables
        self.k = k
        self.pred = predictor
        self.obs = observer
...
"""
Model works, however this model is currently on set up for 1 dimensional data. However, this can be overcome by 
iterating through all dobserver variables and taking the nearest neighbours and average of them to draw a conclusion. 
Important to note the issues that commonly befall KNN apply ehere especially when a deciding class is difficult to determine.
The naive bayes classifier also adheres to its usual restrßictions.
"""
    
"""
Here we call the model with the data specified at the start. We state the columns of the observation and prediction variables we wish to use.
The test set of data is also specified at the start, the example here uses a test set sampled from the raw data, however, the test set 
can be new data as long as it fits the main data frame format.
"""
if __name__ == "__main__":
    run_model = Classifier_model(observer=1, predictor=7, k=11) 
